...
for ifilo = 1:numFilo
    ...
    while testForProj == 0;
        ...
        if count >1
            verticesEP(ifilo,:) = [filoCoords(count-1,1),filoCoords(count-1,2)];
        end
        filoCoords = filoCoords(count:end,:);
        pixIdxBack = pixIdxBack(count:end,:);
        ...
        if length(filoCoords) > filterSize == 1
            ...
            % test to see if ok : should be empty to proceed.
            testForProj = isempty(intersect(pixIdxFor,pixIdxBack)) & length(intersect(indicesForMaskBack(:),indicesForMaskFor(:)))<n;
