...
ip.addParameter('GeoThresh',0); % in cos(thetaRL)
...
[idx,d] = KDTreeBallQuery(endPoints, endPoints, ip.Results.MaxRadiusLargeScaleLink); 
...
% find path between edges: Note this will take out the more egregious paths
% overlapping with the candidate mask : however the path can cross but
% due to pixelation of the paths links can appear to still 'cross' 
% This type of overlap is actually not harmful at this step and get
% facilitate path exploration. If the paths are fruitless they are typically 
% eroded in subsequent steps. (For instance example in S1 segmentation
% movie)
...
if ~isempty(E)
    ...
    idxFilt = (d>ip.Results.MaxRadiusNoGeoTerm & (dotProd21' <=ip.Results.GeoThresh | dotProd12' <=ip.Results.GeoThresh));
    ERemove = E(idxFilt,:);
