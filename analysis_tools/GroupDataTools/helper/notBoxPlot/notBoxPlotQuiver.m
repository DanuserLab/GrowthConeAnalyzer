function varargout=notBoxPlot(y,x,jitter,style,uIn,vIn)
...
for ii=1:length(uX)
    f=find(b==ii);
    h=[h,myPlotter(x(f),y(:,f),uIn(:,f),vIn(:,f))];
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function h=myPlotter(X,Y,uIn,vIn)
h = 1; 
...
for k=1:length(X)
    ...
    thisU = uIn(:,k); 
    thisV = vIn(:,k);
    thisU = thisU(~isnan(thisU)); 
    thisV = thisV(~isnan(thisV)); 
    thisXFinal = thisX'+J'; 
    for l = 1:length(thisU)
       % hq = quiver(thisXFinal(l),thisY(l),thisU(l),thisV(l),0,'color',C,'MaxHeadSize',1/norm([thisU(l),thisV(l)])); 
%         hU = hq.UData; 
%         hV = hq.VData;
%         hX = hq.XData; 
%         hY = hq.YData; 
headLength= 15; 
headWidth = 20; 
   line([thisXFinal(l),thisXFinal(l)+thisU(l)],[thisY(l),thisY(l)+thisV(l)],'color',C); 
   hold on 
  ah  = annotation('arrow'); 
%    ah = annotation('arrow',...
...
% annotation('arrow',[thisXFinal(l),thisXFinal(l)+thisU(l)],[thisY(l),thisY(l)+thisV(l)]); 
 
%  set(ah,'position',[(thisXFinal(l)+thisU(l)) (thisY(l) + thisV(l)) thisU(l) thisV(l)]); 
...
%             'headStyle','cback1','HeadLength',headLength,'HeadWidth',headWidth);
%         set(ah,'parent',gca);
%         set(ah,'position',[(hX(iii,ij)+hU(iii,ij)) (hY(iii,ij)+hV(iii,ij)) hU(iii,ij) hV(iii,ij)]);
% 
%     end
% end
%     end 
       % h(k).data = quiver(thisXFinal,thisY,thisU,thisV,0,'color',C,'MaxHeadSize',inf,'Marker','o','autoscale','off','ShowArrowHead','on');
