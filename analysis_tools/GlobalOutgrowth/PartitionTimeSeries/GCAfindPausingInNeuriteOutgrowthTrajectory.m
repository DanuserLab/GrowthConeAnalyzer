...
ip.addParameter('plotLimsOutgrowth',[]); % for paper -10 to 25 to keep on 
% same axis : for release use the min and max
...
if isempty(ip.Results.plotLimsOutgrowth)
    cLimsDis = [min(d),max(d)];
else 
    cLimsDis = ip.Results.plotLimsOutgrowth; 
...
if ~isempty(outPath)
    ...
    if makePlot
        ...
        hold on
            ...
            arrayfun(@(x) line([trans(x)*secPerFrame-secPerFrame,trans(x)*secPerFrame-secPerFrame],cLimsDis,'color','k'),1:length(trans));
