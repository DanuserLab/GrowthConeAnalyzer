function [filoCurvMax,filoBranchNew] = GCAAnalysisExtract_filoCurvature(filoBranch,filoFilterSet)
%% GCAAnalysisExtract_filoCurvature
% Collects Filopodia Max Curvature
%INPUT:
%
%filoBranch :  REQUIRED:  large rx1 structure (currently takes some time to load)
...
%                       (eventually the filoBranch needs to be
...
for iFrame = 1:nFrames
    ...
    if ~isempty(filoInfo)
        %% Test if need to extract the curvature information and add it to filoInfo
        if ~isfield(filoInfo,'Ext_FiloCurvIndVals')            
            if ~isfield(filoInfo,'Ext_coordsXY_SplineFit')
                filoInfo = GCASplineFitFilopodia(filoInfo);
            end
            filoInfo = GCAAddFilopodiaCurvature(filoInfo);
            if isempty(filoBranchNew)
                filoBranchNew = filoBranch;
            end
            % update filoBranch struct
            filoBranchNew(iFrame).filoInfo = filoInfo;
        end % isfield        
        %% Filter
