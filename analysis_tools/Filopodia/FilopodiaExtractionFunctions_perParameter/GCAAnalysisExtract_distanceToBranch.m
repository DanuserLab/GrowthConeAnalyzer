function [distToBranchCell,filoBranchNew] = GCAAnalysisExtract_distanceToBranch(filoBranch,filoFilterSet,varargin)
%% GCAAnalysisExtract_distanceToBranch
%
%% 
%INPUT: 
...
%     nFrames = 1;
% else
    %nFrames = length(filoBranch);
    % end
    nFrames = numel(filoFilterSet);
% end
distToBranchCell = cell(nFrames,1);
filoBranchNew = [];
%% Extract
for iFrame = 1:nFrames
    ...
    if ~isempty(filoInfo)
        ...
        if ~isempty(filoInfoFilt)
            ...
            distToBranchCell{iFrame} = distValues.*ip.Results.umPerPixel;
            clear distValues
