...
iWinProc = find(cellfun(@(x) sum(strcmpi(x.name_,'Windowing')),movieData.processes_));
%iWinProc = movieData.getProcessIndex('WindowingProcess',3,~p.BatchMode);
iWinProc = iWinProc(end); 
if isempty(iWinProc)
...
for iFrame = 1:(nFrames-1)
    ...
    windInFilePath = movieData.processes_{iWinProc}.funParams_.OutputDirectory; 
    % added MB for window type test 
    save([p.OutputDirectory filesep 'windInFilePath.mat'],'windInFilePath'); 
