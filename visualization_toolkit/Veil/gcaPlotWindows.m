...
ip.addParamValue('lineWidth',0.5);
ip.addParamValue('plotWinds',true); 
ip.parse(varargin{:});
...
switch cellDepth
    ...
    case 1
        ...
        if any(~cellfun(@isempty,windowIn))
            currWin = [windowIn{:}];
            if ~isempty(currWin)
                if ip.Results.plotWinds
                h=patch(currWin(1,:),currWin(2,:),stringIn{:});
                h = plot(currWin(1,:),currWin(2,:),'color',ip.Results.colorWind,'Linewidth',ip.Results.lineWidth);
                end 
            end
        end
            ...
            if ~isempty(windowIn{j})
                ...
                if ~isempty(currWin)
                    if ip.Results.plotWinds
                        ...
                        h(end+1)=plot(currWin(1,:),currWin(2,:),'color',ip.Results.colorWind,'Linewidth',ip.Results.lineWidth);
                    end
