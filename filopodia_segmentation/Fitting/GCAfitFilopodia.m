...
ip.addParameter('forceFitIntoNoise',true); 
ip.addParameter('nPixIntoNoise',3); 
...
%% For Better Background Estimation: Get a list of all the filo pixels detected 
 
 allPixels = arrayfun(@(x) filoInfo(x).Ext_maskIndices(1:length(filoInfo(x).Ext_pixIndicesBack)-1,:),1:length(filoInfo),'uniformoutput',0); 
        ...
        if isempty(intersect(filoInfo(idxCurrent).Ext_pixIndicesFor,filoInfo(idxCurrent).Ext_pixIndicesBack));
            ...
            if ~isempty(xyFilo);
                ...
                if  (~isempty(yData) && length(yData) > 4)
                    ...
                    [slopeMaxNeg,slopeMaxPos,valuesNeg,valuesPos,sd,sv] = gcaFindPotentialSigmoidals(yData,'makePlot',false,'outPath',[p.OutputDirectory filesep toAdd],...
                    ...
                    % filtered image
                    %%
                    indicesBackRect = maskIndices([1:2,4:5],end-nSearch:end);
                    [nyFiloMask,nxFiloMask] = size(maskIndices);
                    
                    indicesBack  = indicesBackRect(:);
                   
                    % take out any detected filopodia pixels from the
                    % background estimation
                    [~,ia,~] = intersect(indicesBack,allPixels)  ;
                    
                    if ~isempty(ia);
                        indicesRemove = indicesBack(ia);    
                        filoInfo(idxCurrent).([toAdd 'indicesRemoveBackground']) = indicesRemove; 
                    end
                    ...
                    filoInfo(idxCurrent).([toAdd 'localBackValues']) = backEst; 
                    ...
                    switch toAdd
                        case 'Ext_'
                            ...
                            % Start correction 20180304
                            if ~isempty(noiseDip)
                                if (ip.Results.forceFitIntoNoise && endFit < noiseDip)
                                    if length(yData)-noiseDip < ip.Results.nPixIntoNoise
                                        endFit = length(yData);
                                    else
                                        endFit = noiseDip + ip.Results.nPixIntoNoise;
                                    end
                                end   % if ip.Results..forceFitIntoNoiseDip
                            end
                            % End correction 
                ...
                if  (~isempty(yDataFit) && length(yDataFit) > 4) % make a cut off that length yData needs to be at least four pixels.
                    ...
                    %% Attempt to refit
                    % added 20180304 to try to fix. indicates the mean is
                    % the start of the yDataFit ... something likely off.
%                     if (pixIdx ~=1 && pixIdx == startFit)
%                         
%                         % test is the maxSlope is greater than the fitPix
%                         % values (added 03082018)
%                         if ~isempty(slopeMaxPos) % test for increasing region
%                             
%                             if ~isempty(test2) % if not empty there is an increasing region in the left hand side
%                                 % part of the fit. ... the bad fit
%                                 % indicates it was likely noise
%                                 if slopeMaxNeg(end)-ip.Results.fitLengthInPix >=1
%                                     startFit = slopeMaxNeg(end)-ip.Results.fitLengthInPix;
%                                 else
%                                     startFit =1;
%                                 end
%                             else
%                                 svTest = sv;
%                                 svTest(1:startFit-1)= NaN;
%                                 svTest(slopeMaxNeg(end)+1:end)=NaN;
%                                 % find the max sv between startFit and maxNegSlope
%                                 
%                                 startFit = find(svTest==max(svTest));
%                             end
%                         else % ~isempty(slopeMaxPos)
%                             svTest = sv;
%                             svTest(1:startFit-1)= NaN;
%                             svTest(slopeMaxNeg(end)+1:end)=NaN;
%                             % find the max sv between startFit and maxNegSlope
%                             
%                             startFit = find(svTest==max(svTest));
%                         end % ~isempty(slopeMaxPos)
%                         
%                         if startFit ~= slopeMaxNeg(end)
%                             % quick and dirty: try to refit
%                             yDataFitOld = yDataFit;
%                             yDataFit = yData(startFit:endFit);
%                             if  (~isempty(yDataFit) && length(yDataFit) > 4)
%                                 distFiloFit = distFilo(startFit:endFit);
%                                 
%                                 starts = [max(yDataFit)-min(yDataFit),length( filoInfo(idxCurrent).([toAdd 'pixIndicesBack'])),5,...
%                                     min(yDataFit)];
...
%                                 clear yDataFitOld;
%                             else
%                                 yDataFit = yDataFitOld; % use the old yData
%                             end % ~isempty(yDataFit)...
%                         end
%                         %find an approximate single value index of the filo tip position
%                         % (as measured along filo: 0  = the base of the filo)
%                         % so can get a quick xy coord for plotting.
...
%                         
%                         pixIdx = find(delt==min(delt));
%                     end % if pixIdx ~=1 && pixIdx == startFit
                    %%
                    ...
                    if ~isempty(slopeMaxNeg)
                        filoInfo(idxCurrent).([toAdd 'slopeMaxNeg']) = [distFilo(slopeMaxNeg),yData(slopeMaxNeg)];
                    else
                        filoInfo(idxCurrent).([toAdd 'slopeMaxNeg']) = [NaN,NaN];
                    end
                    
                    if ~isempty(slopeMaxPos)
                        filoInfo(idxCurrent).([toAdd 'slopeMaxPos']) = [distFilo(slopeMaxPos),yData(slopeMaxPos)];
                    else
                        filoInfo(idxCurrent).([toAdd 'slopeMaxPos']) = [NaN,NaN];
                    end
                    filoInfo(idxCurrent).([toAdd 'coordsForFit']) = [distFiloFit,yDataFit];
                    ...
                    filoInfo(idxCurrent).([toAdd 'slopeMaxNeg']) = [NaN,NaN];
                    filoInfo(idxCurrent).([toAdd 'slopeMaxPos']) = [NaN,NaN];
                    filoInfo(idxCurrent).([toAdd 'coordsForFit']) = [NaN,NaN]; 
                    filoInfo(idxCurrent).([toAdd 'localBackValues']) = NaN; 
                end
            else
                filoInfo(idxCurrent).([toAdd 'params']) = NaN;
                filoInfo(idxCurrent).([toAdd 'endpointCoordFitXY']) =[ NaN,NaN];
