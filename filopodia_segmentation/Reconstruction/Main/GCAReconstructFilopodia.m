...
ip.addParameter('dilateLocalRegion',false);% (true,false, or 'test) flag to dilate further the
% local region of interest estimation (ie decrease the background
% estimation), if true will always dilate, if false, will never dilate, if 
% 'test' will first check the percentage of the image is estimated as
% background and only dilate if the estimated background is larger than
% the threshold set by 'percentBackCutoff'
...
ip.addParameter('filterVeilFromNMSRidgeHist',false); % flag to filter the
% intra-cellular ridge response from the response threshold calc, 
% if not interested in the embedded actin 
% content this is recommended, as there can be strong ridge resonses not of 
% interest, which can make the std of the NMS poorly estimated (usually this
% becomes obvious from just visualization of the histrogram- there are
% extra high response modes localized to the intracellular veil region) 
ip.addParameter('filterBasedOnFloatingVeilPieces',false); 
...
if ip.Results.filterBasedOnFloatingVeilPieces
    veilStemMaskExtra= veilStemMaskAll - veilStemMaskC;   
end 
...
if ip.Results.filterBackEst
    dilationFlag = false;
    if strcmpi(ip.Results.dilateLocalRegion,'test')
        [maskBack,backMu,backSig] = gcaEstimateBackgroundArea(img);
        % test the percentage of the image estimated as background and dilate if
        % greater than the cut-off
        v = maskBack(:);
        percentBack= sum(v==1)/length(v)*100;
        if percentBack>ip.Results.percentBackCutoff
            [maskBack,backMu,backSig] = gcaEstimateBackgroundArea(img,'PostProcess',true);
            maskBack = ~imdilate(~maskBack,strel('disk',ip.Results.LRDilRad)); %
            dilationFlag = true;
        end % if percentBack > ip.Results.percentBackCutoff
        
    elseif islogical(ip.Results.dilateLocalRegion)
        if ip.Results.dilateLocalRegion
            ...
            [maskBack,backMu,backSig] = gcaEstimateBackgroundArea(img);% original verison used for N1E-115 data
        end % ip.Results.dilateLocalRegion
    end % strcmpi
    ...
    if ip.Results.TSOverlaysRidgeCleaning % plot the histogram with cut-off overlay so can see what losing
        ...
        TSFigs(countFigs).name =  'BackgroundEstimation';
        TSFigs(countFigs).group = 'Cleaning_Small_Ridges' ;
        v = maskBack(:); 
        forTitle = sum(v==1)/length(v)*100; 
        n1 = hist(img(:),100); 
        hist(img(:),100);
        hold on
        
        line([backMu+(2*backSig) backMu+(2*backSig)],[0,max(n1)],'color','r','Linewidth',2);
        axis([min(img(:)),max(img(:)),0,max(n1)]);
        if dilationFlag
            toAdd = ['Background Estimate Dilated by ' num2str(ip.Results.LRDilRad)];
        else
            toAdd = [];
        end
        
        title({'Red line: 2*std of first mode'; ['Percent Background ' num2str(forTitle,3)];toAdd});
...
%% filter out intra cellular space if user desires (added 20180321- porcupine many crazy intracellular ridge signal ) 
if ip.Results.filterVeilFromNMSRidgeHist
   maskBack = maskBack |  veilStemMaskC;      
end 
...
if ~isempty(ip.Results.multSTDNMSResponse)
    ...
    cutoffTrueResponse = respNMSMean+ip.Results.multSTDNMSResponse*respNMSSTD; 
else
...
%% 
if ip.Results.filterBasedOnFloatingVeilPieces
 canRidges = canRidges.*~veilStemMaskExtra;    
end 
