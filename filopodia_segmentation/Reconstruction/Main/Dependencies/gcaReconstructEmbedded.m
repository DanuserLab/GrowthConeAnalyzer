...
[ny,nx] = size(img);
TSFigs = [];
%% Check to make sure there are embedded candidates

if sum(embeddedRidgeCand(:))~=0
    ...
    % filter out those CCs with no or more than 2 end points
    weirdSeed = cellfun(@(x) (size(x,1)~=2 || isnan(x(1))),seedFilo1EPs);
    ...
    if ~isempty(idxKeepInt) % if no candidates
        ...
        end % if idxKeepInt %  SEE IF YOU CAN CLEAN THIS UP
    else
        maskPostConnect1 = filoExtSeedForInt;
    end  %isempty(idxKeepInt)
else % no candidates
    maskPostConnect1 = filoExtSeedForInt;
end % if sum(embeddedRidgeCand)
end % function