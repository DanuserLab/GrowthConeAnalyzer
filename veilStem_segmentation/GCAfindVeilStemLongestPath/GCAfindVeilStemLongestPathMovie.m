...
ip.addParameter('neuriteElongTSOutlier_outlierDef_k',9,@(x) isscalar(x)); % see gcaFindOutliersFromMedFilt.m
...
for iCh = 1:nChan
    ...
    display(['Finding Veil/Stem Longest Path for Channel ' num2str(channels(iCh))]);
    ...
    if  exist(veilStemFile,'file')==2
        ...
        load([outDirC filesep 'neuriteLength.mat']);
        ...
        load([ [movieData.outputDirectory_ filesep...
            'SegmentationPackage' filesep 'StepsToReconstruct' filesep 'III_veilStem_reconstruction'] filesep 'Channel_' num2str(channels(iCh))...
    ...
    if strcmpi(ip.Results.EndFrame,'auto')
        endFrame = nFrames;
        display(['Auto End: Finding Neurite Lengths From Frame ' num2str(startFrame) ' to ' num2str(endFrame) 'for Channel' num2str(channels(iCh))]);
    else
        endFrame = ip.Results.EndFrame;
        display(['Manual End: Finding Neurite Lengths From Frame ' num2str(startFrame) ' to ' num2str(endFrame) 'for Channel' num2str(channels(iCh))]);
    end
    ...
    for iFrame = startFrame:endFrame
        ...
        save([outDirC filesep 'veilStem.mat'],'veilStem','-v7.3') ;
        save([outDirC filesep 'neuriteLength.mat'], 'neuriteLength');
    ...
    measC = arrayfun(@(x) neuriteLength(x),1:length(neuriteLength),'uniformoutput',0); 
    measC = measC'; 
    % also save in format of other descriptors so can easily collect. 
    save([measDir filesep 'meas_neuriteLength.mat'], 'measC'); 
    ...
    for ifile = 1:numel(fileType)
        if ip.Results.TSOverlays  
            saveas(TSFig.h,[outDirC filesep TSFig.name fileType{ifile}]);
