function [ veilStem,TSFigs ] = GCAReconstructVeilStem(listOfImages,varargin)
...
ip.addOptional('backboneInfo',[]);
ip.addOptional('BBScales',[]);
%
...
ip.addParameter('threshType','local'); % Intitial Thresholding Method (local), global, external
ip.addParameter('maskDirectory',[]); % if method type external : just load a previously run mask
ip.addParameter('LocalThresholdPatchSize',75,@(x) isscalar(x)); % only necessary for local thresholding
...
ip.addParameter('usePreviousParams',true); 

ip.parse(listOfImages,varargin{:});
p = ip.Results;
...
if ~isempty(p.backboneInfo)
    backboneInfo = p.backboneInfo{1};
    BBScales = p.BBScales{1}; 
end 
...
%     maskDirectory = MD.processes_{idxThresh}.funParams_.OutputDirectory;
...
for iFrame = ip.Results.StartFrame:ip.Results.EndFrame
    ...
    %% 
%     if restart  && p.usePreviousParams 
%         p = paramsArchived
%     end 
...
%                             end
    ...
    veilStem(iFrame).timeStamp = clock;
        ...
        fullMask =  imfill(erodForBody,'holes'); 
        veilStem(iFrame).finalMask = fullMask; 
        %% Make Trouble shoot overlays if applicable.
        if ip.Results.TSOverlays
