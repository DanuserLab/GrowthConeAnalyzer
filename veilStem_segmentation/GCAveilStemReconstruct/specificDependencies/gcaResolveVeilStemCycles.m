...
ip.parse(backbone2Dil,veilStemNodeMask, backboneInfo,img,varargin{:});
p = ip.Results;
%% Initiate
if p.TSOverlays 
    iFig = 1;
...
%     end % TSOverlays
    ...
    if length(edgesStr) ~= length(test) % test for repeats.
        ...
        if p.TSOverlays
            cMap = jet(128);
            cMap = cMap(1:128,:); %
            mapper=linspace(0,3,128)';
            D=createDistanceMatrix(scoresRepeats',mapper);
            [~,idxCMap]=sort(abs(D),2);
            
            TSFigs(iFig).h = setFigure(nx,ny,'off');
            TSFigs(iFig).name = 'Parallel_Connections';
            imshow(-img,[]);
            hold on
            repeats = CCEdges.PixelIdxList(idxTest); 
            for k=1:128
                ...
                if ~isempty(toPlot)
                    for i = 1:size(toPlot,1)
                        [nyC,nxC] = ind2sub(size(img),repeats{toPlot(i)});
        ...
        if p.TSOverlays
            ...
            for k=1:128
                ...
                if ~isempty(toPlot)
                    ...
                    [nyC,nxC] = ind2sub(size(img),CCEdges.PixelIdxList{toPlot(iPlot)});
                    scatter(nxC,nyC,10,cMap(k,:),'filled');
                    end 
