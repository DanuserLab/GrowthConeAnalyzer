...
ip.addParameter('filterBackEst',true); 
ip.addParameter('dilateLocalRegion', false); 
ip.addParameter('LRDilRad',10); 

% iii. THICK RIDGE LINKING 
ip.addParameter('MaxRadiusLargeScaleLink',10) ; % in pixels 
ip.addParameter('MaxDistNoGeoTerm',3); % in pixels 
ip.addParameter('GeoThresh',0);% in cos(theta)
...
if ip.Results.TSOverlays 
    ...
    TSFigs(figCount).name = '01_SigmaOfLargeRidges';
    sigmaMap = (maxNMSLarge>0).*scaleMapLarge;
    nSigmas = length(ip.Results.BBScale);
    cmapS = colormap(jet(nSigmas));
    %imagesc(scaleMapLarge.*(maxNMSLarge>0));
    imshow(-img,[]) ;
    hold on
    for iScale = 1:nSigmas
        idx = find(sigmaMap ==ip.Results.BBScale(iScale));
        [cy,cx]  = ind2sub([ny,nx],idx);
        scatter(cx,cy,10,cmapS(iScale,:),'filled');
    end
    figCount = figCount+1; % figure completed
    % plot the colorbar
    TSFig1H = figure('visible','off');
    TSFigs(figCount).h = TSFig1H;
    TSFigs(figCount).name = '01_SigmaOfLargeRidges_ColorBar';
    dummy = ones(ny,nx);
    dummy(1,1:end) = max(ip.Results.BBScale);
    dummy(2:end,1:end) = min(ip.Results.BBScale);
    imagesc(dummy);
    colormap(cmapS);
    colorbar
    figCount = figCount+1; 
...
if ~ischar(ip.Results.ThreshNMSResponse)
    ...
    cutoff = respNMSMean + 3*respNMSSTD;
end
...
if ip.Results.TSOverlays 
    ...
    TSFigs(figCount).name = '02_HistogramNMSResponse';
    ...
    ylabel('Count'); 
    xlabel('Large Sigma NMS Response Values'); 
...
if ip.Results.TSOverlays
    ...
    TSFigs(figCount).name = '03_RidgeCandBeforeAfterClean';
    
    imshow(-img,[]);
    hold on
    spy(ridgeCandBeforeClean,'b'); 
    hold on  
    % finish overlay
    spy(cleanedRidge,'r'); 
    spy(maskBack,'g'); 
...
% make sure to take out any candidates without endpoints (very rarely
% happens but can make crash 
sanityCheck = cellfun(@(x) isempty(getEndpoints(x,[ny,nx])),CCRidgeBone.PixelIdxList); 
CCRidgeBone.PixelIdxList(sanityCheck) = []; 
CCRidgeBone.NumObjects = CCRidgeBone.NumObjects-sum(sanityCheck); 
...
if ip.Results.TSOverlays 
    ...
    TSFigs(figCount).name = '04_BeforeAndAfterConnect';
...
while isempty(CCBorderRidgeCandPixels)
    ...
    idxRidgeBorder_Logic = (distTransBound<(distFromBorder) & cleanedRidgeLinked ==1); 
...
if ip.Results.TSOverlays
    ...
    TSFigs(figCount).name = '05_CandSeeds';
    imshow(-img,[]);
    hold on
    candidateSeeds = zeros(size(img));
    candidateSeeds(vertcat(CCBorderRidgeCandPixels{:})) = 1;
