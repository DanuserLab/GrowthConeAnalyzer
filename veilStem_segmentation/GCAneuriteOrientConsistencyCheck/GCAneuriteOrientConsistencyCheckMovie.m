...
if isempty(ip.Results.InputDirectory)
   inDir =  [movieData.outputDirectory_ filesep ...
...
for iCh = 1:nChan
    ...
    saveDir = [outDir filesep 'Channel_' num2str(channels(iCh))];
    ...
    inDirFull = [inDir filesep 'Channel_' num2str(channels(iCh)) ];
    load([inDirFull filesep 'backboneInfo.mat']);
    ...
    p.modified = modified;
    %% Troubleshoot Plots
    if (ip.Results.TSOverlays  && ~ isempty(frames2Fix))
        ...
        for iFrame = 1:length(frames2Fix)
            ...
            if ip.Results.screen2png
                ...
                cellfun(@(x) saveas(gcf,[fixDir filesep 'OldVsNew' num2str(frames2Fix(iFrame),'%03d') x]),fileExt);
            end
            close gcf
        end % iFrame
    end % if ip.Results
    ...
    display(['Finished Neurite Orientation Consistency Test : ' movieData.outputDirectory_ ' for Ch_' num2str(channels(iCh)) ]);
end % iCh
end % The END
