...
dilateMask = imdilate(dilateMask,strel('disk',p.SizeOfConsistencyRestraint)); 
if p.CheckOrient 
...
if isempty(frames2Fix)
    ...
    % outliers)
    outlier = round(length(goodFrames)*0.05);
    if outlier >=1
        sumBB(sumBB<=outlier) =0;
    end
    ...
    for iFrame = 1:length(frames2Fix)
        ...
        % make sure all of them have endpoints
         EPsCheck = cellfun(@(x) ~isempty(getEndpoints(x,[ySize,xSize])),CCNMS.PixelIdxList); 
         
         if sum(EPsCheck) ~=length(EPsCheck)
             CCNMS.PixelIdxList= CCNMS.PixelIdxList(EPsCheck);
             CCNMS.NumObjects = CCNMS.NumObjects-sum(~EPsCheck);
         end
